...
| 窗口宽度 | 表现 |
|---|---|
| ≥ 110 列 | 字形放大 2 倍(header 102 列),按钮间距同步拉开 |
| 55 ~ 109 列 | 字形 1 倍(header 51 列) |
| < 55 列 | 退回一行纯文字标题,避免字符画被折行折断 |

内容整体**垂直居中** —— 大屏幕上不会挤在顶部、下面空一片。
窗口太矮放不下时,页脚会自动省略,而不是被挤出屏幕底部。
调整窗口大小会即时重排。

### 改字样 / 调大小

字符画在 `lua/plugins/dashboard.lua` 顶部的 `FONT` 表里,每个字母定义 5 行,
放大靠整数倍复制,所以改字体只需改这张表。改字样只需改 `render('SNAKENVIM')`
里的字符串;`FONT` 里没有的字母会被跳过。

文件顶部有三个开关:

```lua
local ADAPTIVE = true     -- 关掉就固定 1 倍、不垂直居中
local FORCE_SCALE = nil   -- 写 1 或 2 强制字形倍率,nil 为自动
```

字形表是手写的,所以渲染时会自动把每个字母的各行补齐到等宽 ——
某行多打或少打一个空格不会让整幅画错位。

字符用的是 `█` 全角块而不是 Nerd Font 私有码点:任何等宽字体都有这个字形,
iPad 上走 mosh 也不会变成豆腐块。也正因为字形表里只有 `█` 和空格,
整数倍放大才是精确的。

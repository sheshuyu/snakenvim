...
- **立体字样**用 ANSI Shadow: `█` 做填充,`╔╗╚╝═║` 做棱边和投影,
  立体感来自字形本身而不是特效
- **颜色是渐变**:header 自上而下由亮到暗(模拟顶部打光),
  上下装饰带横向渐变 —— 端点颜色都从当前配色方案里取,换主题会自动跟着变
- **选中行整行铺底色**,像 telescope / fzf 的选中行,一眼能看到选的是哪个
- 上下各一条**全宽装饰带**把画面框住:上面是渐变条,下面是波浪线,
  四角用 `╭╮╰╯` 收边
...
内容整体**垂直居中**,不会挤在顶部。窗口太矮时按这个顺序**依次舍弃**:
先去掉上下装饰带,再去掉页脚 —— 而不是让它们被挤出屏幕。调整窗口会即时重排。

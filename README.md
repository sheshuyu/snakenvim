...
| `<S-h>` / `<S-l>` | **顶栏**:上一个 / 下一个缓冲区(按顶栏的视觉顺序走) |
| `<Space>bb` | 切回上一个缓冲区(`e #`,跳的是 alternate file,不是相邻标签) |
| `<Space>bd` / `<Space>bo` | 关闭当前 / 其它缓冲区 |
| `<Space>bp` | **顶栏**:按字母跳转(标签上出现 a/s/d… 按字母直接跳过去) |
| `<Space>cd` | 显示当前行的诊断 |
| `[d` / `]d` | 上/下一个诊断 |
...
界面分上下两层:**顶栏说「开了哪些文件」,状态栏说「当前这个文件怎么样」**。

顶栏把所有打开的 buffer 列成一行标签,当前那个用一条竖线标出来。每个标签上还能看到:
...
> 在 catppuccin 里零命中),用它就得手写约 10 个高亮组才能接上 Trae 配色;
> barbar 的集成用的 `surface1`/`mantle`/`blue`/`yellow` 全在 `color_overrides` 里,
> 配色自动就对。(2) 上游最后推送是 **2025-01**,而 barbar 是 **2026-06**。
>
> 顶栏**不挂** `<Space>uo`(远程卡顿优化):那个开关的定位是关掉「光标一动就重绘」
> 的东西,而顶栏只在 buffer 变化时重绘。它的动画(`animation`)已经常开关闭了。
...
> **状态栏、顶栏与 markdown 渲染里的图标都是特意关掉的。** lualine 的 filetype /
> branch / diff 组件默认会画 Nerd Font 私有区字形(py 是 U+E606、md 是 U+F48A),
> 顶栏默认的关闭按钮同样是私有区,barbar 的 filetype 图标走 devicons,
> markdown 渲染的标题/复选框/代码块图标也是私有区。没有 Nerd Font 的终端上
> 这些全是豆腐块,所以统一关掉、改用文字和通用符号 —— 和下面的缩进线是同一套原则。
>
> 顶栏里**只有 filetype 图标跟随 `<Space>ui` 开关**,分隔符 `▎`(U+258E)、
> 修改标记 `●`(U+25CF)、关闭按钮 `×`(U+00D7) 是各字体都有的字符,永远显示。
...
- **立体字样**用 ANSI Shadow: `█` 做填充,`╔╗╚╝═║` 做棱边和投影,
  立体感来自字形本身而不是特效
- **颜色是渐变**:header 自上而下由亮到暗(模拟顶部打光),顶部是**固定的中性白**
  `#d8d8d8`,往下混向背景色,换主题不会变色

  > 这里原来是从主题的 `Title` 取色,结果标题随主题漂移 —— catppuccin(Trae 配色)
  > 下 `Title = #80bbff`,整个首页变成蓝的;oxocarbon 是单色主题,才是黑白。
  > 首页是每天第一眼看到的东西,不该因为换主题就换一套情绪,所以改成写死。
  > 底色仍是读 `Normal` 的背景(已被压成纯黑),所以不压黑时渐变还能跟着背景走。
  >
  > 菜单那几行**仍然跟随主题**:标签是 `Normal`、引导线是 `Comment`、
  > 快捷键是 `Special`,所以 catppuccin 下快捷键会是粉的(`#f48cca`)。
